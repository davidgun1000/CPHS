function [particles,w,indx,sir_llh]=smc_diffusion_GARCH_factor_SV_mean_approx(y,T,M,N,delta,a,tau,mu,B,ft,num,dim,phi_factor,tau_factor)
...
if num<=dim   
   ...
   w=zeros(T,N);
   indx=zeros(T,N); 
   particles(:,1:N,t)=normrnd(0,sqrt(tau_factor/(1-phi_factor^2)),1,N);
   ...
   for t=2:T
       indx(t,:)=rs_multinomial(w(t-1,:));
       particles(:,1:N,t)=phi_factor*particles(:,indx(t,1:N),t-1)+sqrt(tau_factor)*randn(1,N);
       logw=-0.5*log(2*pi)-0.5.*log(exp(particles(1,:,t)))-0.5.*(1./exp(particles(1,:,t))).*((ft(num-dim,t)).^2);
       w(t,:)=exp(logw-max(logw));
       sir_llh=sir_llh+log(mean(w(t,:)))+max(logw);
       w(t,:)=w(t,:)./sum(w(t,:));
