function [particles,w,indx,sir_llh]=smc_diffusion_GARCH_factor_SV_mean_approx_corr(y,T,M,N,delta,a,tau,mu,B,ft,num,dim,phi_factor,tau_factor,u1,u1_res)
...
if num<=dim   
   ...
   for t=2:((M)*(T-1)+1)
       ...
       z=particles_res;
       %GARCH
       z=z+h.*(a.*(mu-exp(z)).*exp(-z)-(tau/2))+sqrt(tau)*sqrt(h)*u1(:,t)';
       %OU
       %z=z+h*(a*(mu-z))+sqrt(tau)*sqrt(h)*u1(:,t)';
       z=real(z);
       particles(:,1:N,t)=z;
   ...
   w=zeros(T,N);
   indx=zeros(T,N); 
   particles(:,1:N,t)=sqrt(tau_factor/(1-phi_factor^2))*u1(:,t)';
   ...
   for t=2:T
       [indx(t,:)]=rs_multinomial_corr2(particles(:,1:N,t-1),w(t-1,:),u1_res(:,t)');
       particles(:,1:N,t)=phi_factor*particles(:,indx(t,1:N),t-1)+sqrt(tau_factor).*u1(:,t)';
       logw=-0.5*log(2*pi)-0.5.*log(exp(particles(1,:,t)))-0.5.*(1./exp(particles(1,:,t))).*((ft(num-dim,t)).^2);
       w(t,:)=exp(logw-max(logw));
       sir_llh=sir_llh+log(mean(w(t,:)))+max(logw);
       w(t,:)=w(t,:)./sum(w(t,:));
...
%    w=zeros(T,N);
%    indx=zeros(T,N);
%    particles(:,1:N,t)=normrnd(0,1,1,N);
%    logw=-0.5*log(2*pi)-0.5.*log((exp(particles(1,:,t))))-0.5.*(1./(exp(particles(1,:,t)))).*((y(1,t)-B(1,:)*ft(:,t)).^2);
%    w(t,:)=exp(logw-max(logw));
%    sir_llh=log(mean(w(t,:)))+max(logw);
%    w(t,:)=w(t,:)./sum(w(t,:));
