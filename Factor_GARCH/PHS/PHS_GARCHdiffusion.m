...
for i=1:iter
    ...
    for t=1:T
     ...
     parfor s=1:dim_y
         [X,W,A,lik]=csmc_diffusion_GARCH_factor_SV_mean_approx(y(s,:),T,M,N,delta,a(s,1),tau(s,1),mu(s,1),B(s,:),fact_score,ctraj_idio(s,:),s,dim_y,0,0);
         ...
         tau_star=exp(R1(1,2));
         mu_star=R1(1,3);
         [Xstar,Wstar,Astar,lik_star]=smc_diffusion_GARCH_factor_SV_mean_approx(y(s,:),T,M,N,delta,a_star,tau_star,mu_star,B(s,:),fact_score,s,dim_y,0,0);
    ...
    parfor s=dim_y+1:dim_y+num_fact
           [X,W,A,lik]=csmc_diffusion_GARCH_factor_SV_mean_approx(0,T,M,N,delta,0,0,0,0,fact_score,ctraj_factor(s-dim_y,:),s,dim_y,phi_factor(s-dim_y,1),tau_factor(s-dim_y,1)); 
           ...
           R1=mvnrnd(theta,scale_factor(s-dim_y,1).*0.01);
           tau_factor_star=exp(R1(1,1));
           [X_star,W_star,A_star,lik_star]=smc_diffusion_GARCH_factor_SV_mean_approx(0,T,M,N,delta,0,0,0,0,fact_score,s,dim_y,phi_factor(s-dim_y,1),tau_factor_star);
    ...
    toc
     ...
     if i==250 | i==500 | i==1000 | i==2000 | i==3000 | i==4000 | i==5000 | i==6000 | i==7000 | i==10000 | i==12000 | i==14000
        save(['/scratch/rl75/dg2271/standardmix_GARCH_Mix',num2str(N),'.mat'],'Post');
     end
