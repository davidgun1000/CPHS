...
tic
    ...
    parfor s=1:dim_y
        [u1]=obtain_random_numbers(phi(s,1),tau(s,1),mu(s,1),ctraj(s,:),N,T); 
        [X,W,A,lik,u1_res_rand]=csmc_mu_factorSV_corr(y(s,:),B(s,:),fact_score,phi(s,1),tau(s,1),mu(s,1),N,T,ctraj(s,:),u1,s,dim_y);        
        ...
        tau_star=exp(R1(1,1));
        [X_star,W_star,A_star,lik_star]=smc_mu_factorSV_corr(y(s,:),B(s,:),fact_score,phi(s,1),tau_star,mu(s,1),N,T,u1,u1_res_rand,s,dim_y);
