...
nit=11000;
iter=burn+nit;
N=500;%the number of particles
T=length(y_order_demean_used(1,:)');%the length of time series
...
for i=1:iter
    ...
    %sampling the SV parameters mu and phi and tau2 and rho in Gibbs steps
    for s=1:dim_y
         ...
         lik=-((T-1)/2)*log(1-rho(s,1)^2)-0.5.*(1./(tau(s,1)*(1-rho(s,1)^2))).*sum(((ctraj(s,2:T)-mu(s,1))-phi(s,1).*(ctraj(s,1:T-1)-mu(s,1))-rho(s,1).*sqrt(tau(s,1)).*eps).^2)-...
             (T/2)*log(tau(s,1))-0.5.*((1-phi(s,1)^2)/tau(s,1)).*((ctraj(s,1)-mu(s,1)).^2);
         post=prior+lik;
         jac=log(1/tau(s,1));
         theta=[log(tau(s,1)),atanh(rho(s,1))];
         theta_star=mvnrnd(theta,scale_idio(s,1).*V1(:,:,s));
         tau_star=exp(theta_star(1,1));
         rho_star=tanh(theta_star(1,2));
         prior_star=logcauchy(tau_star);
         lik_star=-((T-1)/2)*log(1-rho_star^2)-0.5.*(1./(tau_star*(1-rho_star^2))).*sum(((ctraj(s,2:T)-mu(s,1))-phi(s,1).*(ctraj(s,1:T-1)-mu(s,1))-rho_star.*sqrt(tau_star).*eps).^2)-...
             (T/2)*log(tau_star)-0.5.*((1-phi(s,1)^2)/tau_star).*((ctraj(s,1)-mu(s,1)).^2);
         ...
         if i>250
            ...
            V1(:,:,s)=jitChol(V1(:,:,s));
            scale_idio(s,1)=update_sigma(scale_idio(s,1),C2,target_accept,i,2); 
         end
    end
    ...
    for s=dim_y+1:dim_y+num_fact
       ...
       tau_a=(T-1)/2;
       tau_b=0.5*(1-phi(s,1)^2)*(ctraj(s,1))^2+0.5.*sum((ctraj(s,2:T)-(phi(s,1).*(ctraj(s,1:T-1)))).^2);
       tau_star=1./gamrnd(tau_a,1./tau_b);
       num_tau=(1+tau(s,1));
       den_tau=1+tau_star;
       r2=num_tau/den_tau;
       c2=min(1,r2);
    ...
    parfor s=1:dim_y
        [X,W,A,~]=csmc_mu_factorSV_leverage(y(s,:),B(s,:),fact_score,phi(s,1),tau(s,1),mu(s,1),rho(s,1),N,T,ctraj(s,:),s,dim_y);                
        ctraj(s,:)=backward_simulation_SV_leverage(X,W,A,phi(s,1),tau(s,1),mu(s,1),rho(s,1),y(s,:),B(s,:),fact_score);
    end
     
    parfor s=dim_y+1:dim_y+num_fact
       [X,W,A,~]=csmc_mu_factorSV(0,0,fact_score,phi(s,1),tau(s,1),0,N,T,ctraj(s,:),s,dim_y);       
    ...
    %save the results
     if i==250 | i==500 | i==1000 | i==2000 | i==3000 | i==4000 | i==5000 | i==6000 | i==7000 | i==10000 | i==12000 | i==14000
        save('/short/jz21/dg2271/corr_mix/factor_SV/PG_real_N500_leverage_fourfactor.mat','Post');
     end
     
end
save('/short/jz21/dg2271/corr_mix/factor_SV/PG_real_N500_leverage_fourfactor.mat','Post');

% save('/srv/scratch/z3512791/factor_SV/PG_real_N200_leverage_fourfactor.mat','Post');
% save('corr_factor_real_N100_leverage.mat','Post');
% save('/short/jz21/dg2271/corr_mix/factor_SV/PG_real_N20_leverage_fourfactor.mat','Post');
