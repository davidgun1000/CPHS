...
D2=1;
for i=1:num_fact
    scale_factor(i,1)=0.01;
end
ctraj=[ctraj_idio_true;ctraj_factor_true];
fact_score=fact_true';

%parfor s=1:dim_y
%   [X,W,A]=smc_leverage_mu_faclev_used_mex(y(s,:),B(s,:),fact_score,phi(s,1),log(tau(s,1)),mu(s,1),0,N,T,s,dim_y);
%   ctraj(s,:)=ancestor_trace(X,W,A);
%end
%parfor s=dim_y+1:dim_y+num_fact
%   [X,W,A]=smc_leverage_mu_faclev_used_mex(0,0,fact_score,phi(s,1),log(tau(s,1)),0,0,N,T,s,dim_y);
%   ctraj(s,:)=ancestor_trace(X,W,A);
%end
...
for i=1:iter
    ...
    for s=1:dim_y
        ...
        mu_var=tau(s,1)./((1-phi(s,1)^2)+(T-1)*(1-phi(s,1))^2);
        mu_mean=(mu_var./tau(s,1)).*((ctraj(s,1)*(1-phi(s,1)^2))+sum(ctraj(s,2:T)-phi(s,1).*ctraj(s,2:T)+phi(s,1)^2.*ctraj(s,1:T-1)-phi(s,1).*ctraj(s,1:T-1)));
        mu(s,1)=normrnd(mu_mean,sqrt(mu_var));
        
        phi_var=tau(s,1)./(sum((ctraj(s,2:T)-mu(s,1)).^2)-(ctraj(s,1)-mu(s,1))^2);
        phi_mean=(phi_var/tau(s,1)).*sum(-mu(s,1).*(ctraj(s,1:T-1)-mu(s,1))+ctraj(s,2:T).*(ctraj(s,1:T-1)-mu(s,1)));
    ...
    parfor s=1:dim_y
        [X,W,A,lik]=csmc_mu_factorSV(y(s,:),B(s,:),fact_score,phi(s,1),tau(s,1),mu(s,1),N,T,ctraj(s,:),s,dim_y);        
        prior=logcauchy(tau(s,1));
        post=prior+lik;
        jac=log(1/tau(s,1));
        theta=log(tau(s,1));
        R1=mvnrnd(theta,scale_idio(s,1));
        tau_star=exp(R1(1,1));
        [X_star,W_star,A_star,lik_star]=smc_mu_factorSV(y(s,:),B(s,:),fact_score,phi(s,1),tau_star,mu(s,1),N,T,s,dim_y);
        ...
        ctraj(s,:)=backward_simulation_SV(X,W,A,phi(s,1),tau(s,1),mu(s,1));
        if i>50
...
%     for s=1:dim_y
%         scale(s,1)=update_sigma(scale(s,1),C1,target_accept,i,1);
% %         if i>50
...
%     end
    ...
    parfor s=dim_y+1:dim_y+num_fact
       ...
       jac=log(1/tau(s,1));
       theta=log(tau(s,1));
       R1=mvnrnd(theta,scale_factor(s-dim_y,1));
       tau_star=exp(R1(1,1));
...
%     end
    ...
    Post.ctraj27(i,:)=ctraj(27,id);
    Post.ctraj28(i,:)=ctraj(28,id);
    
    
    
     if i==250 | i==500 | i==1000 | i==2000 | i==3000 | i==4000 | i==5000 | i==6000 | i==7000 | i==10000 | i==12000 | i==14000
        save('/short/jz21/dg2271/corr_mix/factor_SV/mix_factor_sim_N100.mat','Post');
     end
